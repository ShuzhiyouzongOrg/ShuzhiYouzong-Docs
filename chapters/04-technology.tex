...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm]
    \node[core] (data) at (0,0) {统一地图状态\\点位/路线/选中项};
    \node[core] (check) at (3.4,0) {能力检查\\网络/Key/容器};
    \node[coregold] (online) at (7.0,1.35) {腾讯地图JS GL\\在线标准底图};
    \node[coregold] (local) at (7.0,-1.35) {Leaflet/手绘图\\本地可用路径};
    \node[core] (result) at (10.8,0) {连续导览\\点位/路线/定位};
    \draw[flow] (data.east) -- (check.west);
    \draw[flowgold] (check.north east) to[out=20,in=180] node[above,font=\small]{在线且初始化成功} (online.west);
    \draw[flow] (check.south east) to[out=-20,in=180] node[below,font=\small]{离线/缺Key/异常} (local.west);
    \draw[flowgold] (online.east) to[out=0,in=25] (result.north west);
    \draw[flow] (local.east) to[out=0,in=-25] (result.south west);
    \draw[curved] (online.south) to[out=250,in=70] node[right,font=\scriptsize,align=center]{offline或SDK失败\\恢复本地层} (local.north);
  \end{tikzpicture}}
