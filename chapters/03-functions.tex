...
\begin{figure}[H]
  \centering
  \IfFileExists{figures/login.png}{%
    \includegraphics[width=0.95\textwidth]{figures/login.png}%
  }{%
...
页面右上角提供\textbf{统一语言入口}，语言选择作用于游客导览、知识库、数字人配置、运营大屏、会话反馈和系统管理等\textbf{全部路由}，而不是只替换顶部导航。切换动作同时更新\textbf{Vue I18n活动语言与Pinia用户状态}以及HTML的\filepath{lang}属性，并通过版本化本地存储在下次访问时恢复。业务文案采用\textbf{五元组目录}维护，新增功能必须同时提供五种译文，降低遗漏单个页面或弹窗的风险。
...
\begin{figure}[H]
  \centering
  \IfFileExists{figures/visitor-guide.png}{%
    \includegraphics[width=0.95\textwidth]{figures/visitor-guide.png}%
  }{%
    \begin{tikzpicture}[x=1cm,y=1cm]
...
图\ref{fig:web-real-screens}和图\ref{fig:harmony-real-screens}均来自当前可运行系统，而非概念效果图。Web端展示数字人问答与景区手绘地图上的真实点位；HarmonyOS截图来自HAP在Pura 90模拟器的安装运行结果，覆盖首页、地图与AI导游主链路。Android APK与微信小程序的预留截图位置统一放在第\ref{sec:multiplatform-delivery}节，后续只需按约定文件名替换，无需重新排版。
...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{0.30\textwidth}
    \centering
    \includegraphics[width=\linewidth]{visitor-harmony-home.jpeg}
    \small 首页与景点入口
  \end{minipage}\hfill
  \begin{minipage}[t]{0.30\textwidth}
    \centering
    \includegraphics[width=\linewidth]{visitor-harmony-map.jpeg}
    \small 定位、点位与路线地图
  \end{minipage}\hfill
  \begin{minipage}[t]{0.30\textwidth}
    \centering
    \includegraphics[width=\linewidth]{visitor-harmony-ai.jpeg}
    \small AI导游与数字人入口
  \end{minipage}
...
\begin{figure}[H]
  ...
  \IfFileExists{web-operations-dashboard.png}{%
    \includegraphics[width=0.95\textwidth]{web-operations-dashboard.png}%
  }{\IfFileExists{figures/admin-dashboard.png}{%
    \includegraphics[width=0.95\textwidth]{figures/admin-dashboard.png}%
  }{%
    \begin{tikzpicture}[x=1cm,y=1cm]
